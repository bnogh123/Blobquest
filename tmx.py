...
class Tile(object):
    ...
    def fromSurface(cls, surface):
        ...
        Its tile_width and tile_height will be set
        using the Surface dimensions.'
        Its gid will be 0.'''
...
visible: Whether the object is shown (1) or hidden (0). Defaults to 1.
    ...
    def __repr__(self):
        if self.tile:
            num = (self.px, self.py, self.width, self.height, self.gid)
            return '<Object %s,%s %s,%s tile=%d>' % num
        else:
            num = (self.px, self.py, self.width, self.height)
            return '<Object %s,%s %s,%s>' % num
...
class ObjectLayer(object):
    ...
    def collide(self, rect, propname):
        ...
        '''
        r = []
        for object in self.get_in_region(rect.left, rect.top,
                                         rect.right, rect.bottom):
...
class TileMap(object):
    ...
    def set_focus(self, fx: object, fy: object, force: object = False) -> object:
        ...
        out-of-bounds data, as defined by their dimensions
        ...
        if self.px_width <= w:
            ...
            if (fx - w2) < 0:
                ...
                restricted_fx = self.px_width - w2
                # hit maximum X extent
            else:
                restricted_fx = fx
        if self.px_height <= h:
            # this branch for centered view and no view jump when
